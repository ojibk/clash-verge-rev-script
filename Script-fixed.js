...
function main(config) {
    ...
    //   \u2028-\u202E   行/段终止符 + Bidi 嵌入/弹出/覆盖控制符（连续 7 个码点，三种语义，已合并为单一范围）：
    ...
    try {
        ...
        if (ENABLE_PROCESS_RULE) {
            ...
            // pushLayer 内部为 for...of 迭代，对空数组零次迭代（no-op），与 pushSuffix/pushDomain 行为一致，
            // 无需 length 守卫（保持形式对称，便于将来条目变动时无需同步修改防御逻辑）。
